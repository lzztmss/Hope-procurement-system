...
function sanitizeStateForClient(state, users) {
  const next = { ...(state || {}) };
  // 账号基础字段来自 users 表；个性化模块/流程权限保存在业务状态中，不能被覆盖丢失。
  const storedUsers = new Map((next.users || []).filter((user) => user?.id).map((user) => [user.id, sanitizeUser(user)]));
  next.users = (users || next.users || []).map((user) => ({
    ...(storedUsers.get(user.id) || {}),
    ...sanitizeUser(user),
  }));
  return next;
}

async function listUsers(executor = getPool()) {
...
async function readState() {
  ...
  const users = await listUsers();
  // 库存流水以领域表为准。历史迁移时流水已进入 inventory_movements，
  // 但旧 app_state.inventoryLogs 可能不完整；直接读取状态字段会让页面丢失历史。
  const movements = await listInventoryMovements();
  return sanitizeStateForClient({ ...(result.rows[0].data || {}), inventoryLogs: movements }, users);
}

async function listInventoryMovements() {
  const result = await query(`SELECT id, inventory_id, action, quantity, before_stock, after_stock,
    source_id, source_module, operator_id, occurred_at, data
    FROM inventory_movements
    ORDER BY occurred_at DESC NULLS LAST, id DESC`);
  return result.rows.map((row) => ({
    ...(row.data || {}),
    id: row.id,
    itemId: row.inventory_id || row.data?.itemId || "",
    action: row.action || row.data?.action || "",
    quantity: Number(row.quantity || 0),
    beforeStock: Number(row.before_stock || 0),
    afterStock: Number(row.after_stock || 0),
    sourceId: row.source_id || row.data?.sourceId || "",
    sourceType: row.source_module || row.data?.sourceType || "",
    operatorId: row.operator_id || row.data?.operatorId || "",
    createdAt: row.occurred_at?.toISOString?.() || row.data?.createdAt || "",
  }));
}

async function writeUsers(users, executor = getPool()) {
  if (!Array.isArray(users)) return;
  const incomingIds = [];
  for (const user of users) {
    if (!user.id || !user.phone || !user.name) continue;
    incomingIds.push(user.id);
    const existing = await executor.query("SELECT password_hash FROM users WHERE id = $1", [user.id]);
  ...
  if (incomingIds.length) {
    await executor.query("DELETE FROM users WHERE NOT (id = ANY($1::text[]))", [incomingIds]);
  }
}
...
async function writeState(state, expectedRevision = null) {
  ...
  try {
    ...
    await writeUsers(state.users || [], client);
    ...
    if (expectedRevision === null || expectedRevision === undefined) {
      ...
      const updated = await client.query(
        `UPDATE app_state
         ...
         WHERE id = $1 AND COALESCE((data->'meta'->>'revision')::int, 0) = $3`,
        [STATE_ID, JSON.stringify(safeState), Number(expectedRevision)]
      );
      if (!updated.rowCount) {
        const latest = await client.query("SELECT COALESCE((data->'meta'->>'revision')::int, 0) AS revision FROM app_state WHERE id = $1", [STATE_ID]);
        const error = new Error("数据已在其他页面更新，请刷新后再操作。");
        error.code = "DATA_OUTDATED";
        error.revision = Number(latest.rows[0]?.revision || 0);
        throw error;
      }
    }
    await syncDomainState(client, safeState);
