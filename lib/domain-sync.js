const TABLES = [
  ...
  "CREATE UNIQUE INDEX IF NOT EXISTS uq_sales_orders_code ON sales_orders(code) WHERE code IS NOT NULL",
  "CREATE UNIQUE INDEX IF NOT EXISTS uq_purchase_orders_code ON purchase_orders(code) WHERE code IS NOT NULL",
  "CREATE UNIQUE INDEX IF NOT EXISTS uq_deliveries_code ON deliveries(code) WHERE code IS NOT NULL",
];
...
async function syncDomainState(poolOrClient, state) {
  // Pool 有 connect()，已借出的 Client 同时也有 connect()；只有后者带 release()。
  const ownsClient = typeof poolOrClient.release !== "function";
