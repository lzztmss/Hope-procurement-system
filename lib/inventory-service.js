...
async function confirmDeliveryOutbound({ deliveryId, operatorId }) {
  ...
  const items = Array.isArray(delivery.items) && delivery.items.length ? delivery.items : [{ inventoryId: delivery.inventoryId, quantity: delivery.quantity }];
  if (!items.length || items.some((line) => !line.inventoryId || Number(line.quantity || 0) <= 0)) throw new Error("出库单缺少有效产品或数量");
  return adjustInventoryBatch({
