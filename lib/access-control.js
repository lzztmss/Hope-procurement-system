const ROLE_DEFAULTS = {
  ...
  custom: { scope: "本人", modules: ["dashboard"], actions: ["view"] },
};
...
// 工作台统一展示经营概览。它由服务端从完整状态投影出来；各模块的原始单据
// 仍会按角色裁剪，不能通过工作台取得联系人、电话、地址、设备号、备注等明细。
function dashboardSummary(state) {
  const pick = (record, fields) => Object.fromEntries(fields
    .filter((field) => Object.prototype.hasOwnProperty.call(record || {}, field))
    .map((field) => [field, record[field]]));
  const list = (collection, fields) => (state?.[collection] || []).map((record) => pick(record, fields));
  return {
    leads: list("leads", ["id", "code", "customer", "stage", "status", "owner", "dueDate", "receivedAt", "createdAt", "updatedAt"]),
    purchases: list("purchases", ["id", "code", "project", "product", "quantity", "status", "owner", "lockedDate", "requiredDate", "estimatedDate", "createdAt", "updatedAt"]),
    inventory: list("inventory", ["id", "productId", "name", "model", "stock", "locked", "inTransit", "safeStock", "eta", "updatedAt"]),
    deliveries: list("deliveries", ["id", "code", "project", "inventoryId", "product", "quantity", "status", "owner", "training", "date", "createdAt", "updatedAt"]),
    aftersales: list("aftersales", ["id", "code", "customer", "type", "priority", "status", "owner", "techOwner", "promiseAt", "date", "createdAt", "updatedAt"]),
    notices: list("notices", ["id", "title", "status", "owner", "publisher", "dueDate", "date", "createdAt", "updatedAt"]),
  };
}

function filterStateForUser(state, user) {
  const next = { ...(state || {}) };
  next.dashboardSummary = dashboardSummary(state);
  // 表单需要判断哪些产品已建立库存档案，但不应因此向无库存查看权的账号
  // 泄露数量、库位、锁定数等库存明细。仅下发产品 ID 列表。
  next.availableInventoryProductIds = [...new Set((state?.inventory || [])
    .map((item) => item?.productId)
    .filter(Boolean))];
  // 业务流程（售后换货、销售核验等）需要看到真实可用库存，才能避免把有货
  // 误判为缺货。它不等于开放库存台账页面：成本、单价、库位、供应商等字段
  // 不会下发；用户也不能因此进入库存模块管理数据。
  next.formInventory = (state?.inventory || []).map(({
    id, productId, name, model, stock, locked, inTransit, safeStock, eta,
  }) => ({ id, productId, name, model, stock, locked, inTransit, safeStock, eta }));
  // 即使账号没有“产品字典”页面权限，业务表单（培训、售后等）仍需要
  // 选择产品和规格。仅给出已启用且已建库存档案的最小字段，不能借此管理产品。
  next.formProducts = (state?.products || [])
    .filter((product) => product?.status !== "停用" && next.availableInventoryProductIds.includes(product.id))
    .map(({ id, name, model, unit, category, status }) => ({ id, name, model, unit, category, status }));
  // 类别需要供有产品/库存权限的员工选择，但只能由系统管理员维护。
  next.productCategories = [...(state?.productCategories || [])];
  // 业务字典会被表单作为下拉选项读取，但配置维护只允许系统管理员。
  next.systemDictionaries = { ...(state?.systemDictionaries || {}) };
  for (const [collection, moduleId] of Object.entries(COLLECTION_MODULES)) {
    ...
    if (!canAccessModule(state, user, moduleId, "view")) {
      // 销售可从本人有权查看的销售订单进入其关联出库单；这只支持“查看出库单”
      // 的流程闭环，不授予出库模块菜单、通用编辑或其他出库单的数据访问权。
      if (collection === "deliveries" && user?.role === "sales") {
        next.deliveries = (state.deliveries || []).filter((delivery) => {
          const order = (state.salesOrders || []).find((item) => item.id === delivery.sourceSalesOrderId || item.deliveryId === delivery.id);
          return order && canSeeRecord(state, user, order);
        });
        continue;
      }
      next[collection] = [];
...
function assertStateWriteAccess(user, currentState, incomingState) {
  ...
  if (user?.role !== "admin" && currentState?.systemDictionaries && typeof currentState.systemDictionaries === "object"
    && !sameRecord(currentState.systemDictionaries, incomingState?.systemDictionaries)) {
    throw new Error("只有系统管理员可以维护系统配置");
  }
  for (const [collection, moduleId] of Object.entries(COLLECTION_MODULES)) {
...
function mergeStateForUser(currentState, incomingState, user) {
  ...
  next.systemDictionaries = currentState.systemDictionaries || {};
  for (const [collection, moduleId] of Object.entries(COLLECTION_MODULES)) {
    const current = mapById(currentState[collection]);
    const incoming = mapById(incomingState[collection]);
