...
function createPurchase(state, order, shortages, operatorId) {
...
function createSalesWorkflowRoute({ sendJson, requireUser, readState, writeState, canPerformWorkflow }) {
  ...
  async function handle(req, res) {
    const matched = req.url.match(/^\/api\/sales-orders\/([^/]+)\/(approve|check-inventory)$/);
    if (req.method !== "POST" || !matched) return false;
    const user = await requireUser(req, res);
    if (!user) return true;
    const [, encodedId, action] = matched;
    const state = await readState();
    const requiredAction = action === "approve" ? "sales_approve" : "inventory_check";
    ...
    logAction(state, user.id, "inventory_shortage", "salesOrders", order.id, `有 ${check.shortages.length} 项产品库存不足，已生成采购申请 ${purchase.code}`);
    await saveWorkflow(state, revision);
    sendJson(res, 200, { ok: true, status: order.status, purchaseId: purchase.id, message: "审批通过，库存不足，已生成采购申请" });
    return true;
  }
